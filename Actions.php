...
Class Actions extends DBConnection{
    ...
    function login()
    {
        ...
        $mail = $this->db->query($sql)->fetch_assoc();
        if (!$qry) {
            $resp['status'] = "failed";
            $resp['msg'] = "Invalid username or password.";
        } else {
            
        $verify = $mail['twofactor'];
            if ($verify) {
                
        $otp = rand(000000, 999999);
                $this->db->query("UPDATE user_list set last_otp=$otp where username = '{$username}' and `password` = '" . md5($password) . "'");
                $_SESSION['temp_otp']=$otp;
                $resp['status'] = "otp";
                $resp['email'] = $mail['email'];
                $resp['temp_otp'] =$_SESSION['temp_otp'];
                $resp['name'] =$mail['fullname'];
            } else {
                $resp['status'] = "success";
    ...
    function payment_status()
    {
        ...
        $sql = "SELECT * FROM payment_status where payment_id = '{$payment_id}' ";
        @$qry = $this->db->query($sql)->fetch_array();
        if (!$qry) {
            $resp['status'] = "failed";
        } else {
    ...
    {
        $payment_id = $_GET['payment_id'];
        $sql = "INSERT INTO payment_status (payment_id) VALUES ('$payment_id')";
        ...
        if (!$qry) {
            $resp['status'] = "failed";
            $resp['message'] = "MySQL error: " . mysqli_error($this->db);
        } else {
            $resp['status'] = "success";
        }
    ...
    function verify_otp()
    {
        extract($_GET);
        $otp = $_GET['last_otp'];
        $sql = "SELECT * FROM user_list where last_otp='$otp'";
        @$qry = $this->db->query($sql)->fetch_array();
        if (!$qry) {
            $resp['status'] = "failed";
    ...
    function save_product(){
        ...
        if(isset($cols) && isset($vals)){
            $cols_join = implode(",",$cols);
            $vals_join = implode(",",$vals);
            $vals_join_500 = implode(",",$vals_500);
            $vals_join_250 = implode(",",$vals_250);

            //$vals_join_500 = str_replace("1kg","500gm",implode(",",$vals));
        }
        if(empty($id)){
            $sql = "INSERT INTO `product_list` ({$cols_join}) VALUES ($vals_join),($vals_join_500),($vals_join_250)";
           // echo "<br>",$sql;
        }else{
...
switch($a){
    ...
    case 'verify_otp':
        ...
        break;
     case 'payment_status':
        echo $action->payment_status();
        break;
     case 'set_payment_status':
        echo $action->set_payment_status();
        break;
    case 'customer_login':
