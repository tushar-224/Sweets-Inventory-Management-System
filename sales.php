<div class="card shadow rounded-0">
    <div class="card-body">
        <div class="w-100 h-100 d-flex flex-column">
            <div class="row">
                <div class="col-8">
                    <strong><h3>Transaction</h3></strong>
                </div>
            ...
            <div class="col-12 flex-grow-1">
                <form action="" class="h-100" id="transaction-form">
                    <div class="w-100 h-100 mx-0 row row-cols-2" id="salesbg">
                        <div class="col-8 h-100 pb-2 d-flex flex-column">
                            <div>
                                <h3 class="text-dark">Please select product below</h3>
                            </div>
                            <div class="flex-grow-1 d-flex flex-column bg-light bg-opacity-50">
                                <div class="form-group py-2 d-flex border-bottom">
                                    <label for="search" class="col-auto px-2 fw-bolder text-dark">Search</label>
                                    ...
                                    <div class="flex-grow-1 col-auto pe-2">
                                        <select class="form-control" id="select_weight">
                                            <option value="1kg">1 kilo gram</option>
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr class="item" data-id="<?php echo $row['product_id'] ?>">
                                                <td class="pname"><?php echo $row['cname'] ?> <?php echo $qty == 0 ? '<span class="badge rounded-pill bg-danger">Out of stock</span>' : '' ?></td>
                                                ...
                                                <input type="hidden" class="item_qty" value="<?php echo $qty ?>">
                                            </tr>
                            ...
                            <div class="h-100 d-flex flex-column">
                                <div class="w-100 flex-grow-1">
                                    <div class="h-100 d-flex w-100 flex-column">
                                        <div class="d-flex">
                                            <div class="fs-5 fw-bolder text-dark flex-grow-1">Items</div>
                                            <div class="col-auto">
                                        ...
                                        <div class="pt-2">
                                            <div class="w-100 mx-0 d-flex pb-1">
                                                <div class="col-4 pe-2 fs-6 fw-bolder text-dark">Total MRP₹<br>(Inclusive of GST.)<br/></div>
                    ...
                    <input type="hidden" id="input_payment_method" name="payment_method" value="cash">
                </form>
            </div>
        </div>
    </div>
...
<script>
    $(function() {
        ...
        $('#plist tbody tr').click(function() {
            ...
            var ntr = $("<tr class='bill' tabindex='0'>")
            ...
            ntr.append('<td class=" align-middle text-end main" id="main">' + parseFloat(gstcal).toFixed(2) + '</td>')
    ...
    function compute(_this, gst) {
        ...
        _this.find('[name="quantity[]"]').on('input keydown', function() {
            ...
            var original = parseInt($("tr.item[data-id='"+id+"'] .item_qty").val()) - parseInt(qty);
         
          if(original>=0){

            $("tr.item[data-id='"+id+"'] td.qty").text((original));
            _this.find('.total').text(parseFloat(_total).toLocaleString('en-US', {
                style: 'decimal',
                maximumFractionDigits: 2
            }))
            _this.find('.main').text(parseFloat(main).toFixed(2))
          } else {
            _this.find('[name="quantity[]"]').val(qty-1);
                alert("This product is out of stock.");
                return false;
          }
        ...
        _this.on('focusin', function() {
            $("#item-list tr.bg-warning").removeClass("bg-warning");
            $(this).addClass("bg-warning bg-opacity-50 selected-item")
    ...
    function remove_item() {
        var id = $('#item-list tr.selected-item').attr('data-id');
        qtyMin(id);
    ...
    function qtyMin(id){
        var original = parseInt($("tr.item[data-id='"+id+"'] td.qty").text());
        var qty = parseInt($("tr.bill[data-id='"+id+"'] .quantity-input").val());
        $("tr.item[data-id='"+id+"'] td.qty").text((original+qty));
    }

    $('#plist tbody tr .weight').each(function() {
            var getWeight = $(this).text();
            console.log(getWeight);
            if(getWeight =='500gm' || getWeight == '250gm'){
